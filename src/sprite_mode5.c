...
void init_ghost_data(void) {
    // Red (Blinky - Ghost 0): start at (23, 14), bounce between (23, 14) and (23, 16), initial DIR_DOWN
    ...
    ghosts[0].dir = DIR_DOWN;
    ghosts[0].frame = 10;
    ghosts[0].in_house = true;
    ghosts[0].min_home_py = 14 * MAZE_TILES_SIZE_PX; // (23, 14) -> 112px
    ghosts[0].max_home_py = 16 * MAZE_TILES_SIZE_PX; // (23, 16) -> 128px
    ghosts[0].state = 0; // GHOST_STATE_HOME_BOUNCE
    ghosts[0].mode = 0;  // CHASE
    ghosts[0].bounce_dist_px = 0; // Must complete bounce down to (23,16) and back to (23,14) before exiting
    ghosts[0].sub_px = ghosts[0].world_px << 8;
    ghosts[0].sub_py = ghosts[0].world_py << 8;

    // Pink (Pinky - Ghost 1): start at (23, 16), bounce between (23, 14) and (23, 16), initial DIR_UP
    ...
    ghosts[1].min_home_py = 14 * MAZE_TILES_SIZE_PX; // (23, 14) -> 112px
    ghosts[1].max_home_py = 16 * MAZE_TILES_SIZE_PX; // (23, 16) -> 128px
    ghosts[1].state = 0; // GHOST_STATE_HOME_BOUNCE
    ghosts[1].mode = 0;  // CHASE
    ghosts[1].bounce_dist_px = 0; // Must complete bounce cycle before exiting
    ghosts[1].sub_px = ghosts[1].world_px << 8;
    ghosts[1].sub_py = ghosts[1].world_py << 8;

    // Cyan (Inky - Ghost 2): start at (21, 15), bounce between (21, 14) and (21, 16), initial DIR_DOWN
    ...
    ghosts[2].min_home_py = 14 * MAZE_TILES_SIZE_PX; // (21, 14) -> 112px
    ghosts[2].max_home_py = 16 * MAZE_TILES_SIZE_PX; // (21, 16) -> 128px
    ghosts[2].state = 0; // GHOST_STATE_HOME_BOUNCE
    ghosts[2].mode = 0;  // CHASE
    ghosts[2].bounce_dist_px = 0; // Must complete bounce cycle before exiting
    ghosts[2].sub_px = ghosts[2].world_px << 8;
    ghosts[2].sub_py = ghosts[2].world_py << 8;

    // Orange (Clyde - Ghost 3): start at (25, 15), bounce between (25, 14) and (25, 16), initial DIR_DOWN
    ...
    ghosts[3].dir = DIR_DOWN;
    ghosts[3].frame = 34;
    ghosts[3].in_house = true;
    ghosts[3].min_home_py = 14 * MAZE_TILES_SIZE_PX; // (25, 14) -> 112px
    ghosts[3].max_home_py = 16 * MAZE_TILES_SIZE_PX; // (25, 16) -> 128px
    ghosts[3].state = 0; // GHOST_STATE_HOME_BOUNCE
    ghosts[3].mode = 0;  // CHASE
    ghosts[3].bounce_dist_px = 0; // Must complete bounce cycle before exiting
